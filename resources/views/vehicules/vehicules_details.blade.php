...
<div class="container mt-4">
    ...
    <ul class="list-group mb-3">
        ...
        <li class="list-group-item"><strong>Prix :</strong> {{ $vehicule->prix_par_jour ?? $vehicule->prix_jour }} €/jour</li>
    </ul>
    ...
    <div class="mb-4">
        ...
        @forelse($vehicule->avis as $avis)
            <div class="border p-2 mb-2 rounded">
                <strong>{{ $avis->utilisateur->name ?? $avis->auteur ?? 'Anonyme' }}</strong> :
    ...
    @auth
        <div class="mb-4">
            ...
            <form method="POST" action="{{ route('avis.store') }}">
                ...
                <div class="mb-2">
                    <input type="text" name="auteur" class="form-control" placeholder="Votre nom (optionnel)" value="{{ Auth::user()->name }}">
                </div>
                <div class="mb-2">
    ...
    <!-- Bouton Réserver -->
    @auth
        <a href="{{ route('reservations.create', $vehicule->id) }}" class="btn btn-reserver">Réserver ce véhicule</a>
    @else
        <a href="{{ route('login') }}" class="btn btn-outline-primary">Se connecter pour réserver</a>
    @endauth
</div>
